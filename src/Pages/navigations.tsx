...
const Navigations = () => {
  ...
  return (
    <div className="gap-3 p-5 w-full flex flex-wrap justify-center">
      {memoizedSidebarItems.map((item, i) => (
        <div
          key={i}
          onClick={() => navigate(`/${item.title.toLocaleLowerCase()}`)}
          className={`overflow-hidden relative dark:hover:bg-[#050708]/ p-4  w-full sm:w-1/4 rounded-lg cursor-pointer border border-gray-600/60 dark:bg-gray-700/20 dark:border-gray-700/50`}
        >
          {/* {i === 4 ? (
